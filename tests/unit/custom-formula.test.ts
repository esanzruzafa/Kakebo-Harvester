...
describe("custom export formulas", () => {
  it("evaluates arithmetic, source values, and approved functions", () => {
    ...
    expect(evaluateCustomFormula("4604895721461865 / 2826355478429782", {}))
      .toBe(1.629269834104581);
    expect(evaluateCustomFormula("1 / 1048576 / 1048576", {})).toBe(2 ** -40);
  ...
  it("rejects syntax outside the limited expression language", () => {
    ...
    expect(() => customFormulaSchema.parse("(9007199254740991 * 2) / 2")).not.toThrow();
    expect(() => customFormulaSchema.parse("1 / 1048576 / 1048576")).not.toThrow();
